    ...
    void time_DTF(int N, int num_threads, int iteration);
    ...
    #endif
        int iteration = 10;
        time_DTF(10000, 1, iteration );
        time_DTF(10000, 32, iteration );
        time_DTF(10000, 64, iteration );
        time_DTF(10000, 128, iteration );
    ...
    void time_DTF(int N, int num_threads, int iteration) {
        ...
        setOutputZero(Xr_o, Xi_o, N);
        int idft = 1;
        double timing_array[iteration];
        
        for (int i = 0; i < iteration; i++) {
            double start_time = omp_get_wtime();
            DFT(idft, xr, xi, Xr_o, Xi_o, N);
            timing_array[i] = omp_get_wtime() - start_time;
        }
        double mean_time = 0;
        double standard_deviation = 0;

        for (int i = 0; i < iteration; i++) {
            mean_time += timing_array[i];
        }
        mean_time /= iteration;
        for (int i = 0; i < iteration; i++) {
            standard_deviation += pow(timing_array[i] - mean_time, 2);
        }
        standard_deviation /= iteration;
        standard_deviation = sqrt(standard_deviation);
        printf("N = %d, num_threads = %d, mean_time = %f, standard_deviation = %f\n", N, num_threads, mean_time, standard_deviation);
